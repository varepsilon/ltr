...
{
  if(ioPopulation.size() == 0 || inNumberParticipants == 0) return;

  std::vector<unsigned int> lIndices(ioPopulation.size(), 0);
  for(unsigned int i=0; i<ioPopulation.size(); i++) {
    unsigned int lChoosenIndividual = i; //ioContext.mRandom(ioPopulation.size());
    for(unsigned int j=1; j<inNumberParticipants; ++j) {
      unsigned int lTriedIndividual = i;
      while (lTriedIndividual == i) {
        lTriedIndividual = ioContext.mRandom(ioPopulation.size());
      }
      //ioContext.mRandom(ioPopulation.size());
