...
namespace logger {
  ...
  Logger::Logger() {  // impossible is nothing
  }

  class Logger::Sync::Mutex {
  public:
    interprocess_mutex instance;
  };
  ...
  void Logger::Init(const string& logFile, LogLevel level) {
    Close();
    {
      scoped_lock< interprocess_mutex > lock(sync.mutex->instance);
  ...
  void Logger::LogMessage(Logger::LogLevel level,
  ...
  LogStream::LogStream(Logger::LogLevel level,
                       const string& alias,
                       bool echoInStd,
                       Logger& logger) :
  level_(level),
  ...
  const LogStream&
  LogStream::operator<<(std::ostream& (*p)(std::ostream&)) const {
  ...
  PrintLogger::PrintLogger(const string& alias,
                           bool echoInStd, Logger& logger) :
  alias_(alias),
  echoing_(echoInStd),
