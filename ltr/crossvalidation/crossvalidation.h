...
namespace ltr {
  namespace cv {
    ...
    const ValidationResult& Validate(
      ...
      for (int split_index = 0; split_index < splitter.splitCount(); ++split_index) {
        ...
        for (int measure_index = 0; measure_index < measures.size(); ++measure_index) {
          current_measure_values.push_back(measures[measure_index].average(current_splitted.test_set));
        }
