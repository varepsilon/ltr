...
template<class TValue>
typename DecisionVertex<TValue>::Ptr
  DecisionVertexPtr(Condition::Ptr condition) {
...
template <class TValue>
TValue DecisionVertex<TValue>::value(const Object& object) const {
  ...
  while (child != NULL) {
    double cond_value = child->condition()->value(object);
...
template <class TValue>
string DecisionVertex<TValue>::generateCppCode(
  const string& function_name) const {
    ...
    if (child != NULL) {
      ...
      hpp_code.append(child->condition()->getDefaultSerializableObjectName());
      child = child->nextSibling();
    }
    while (child != NULL) {
      hpp_code.
    ...
    hpp_code.
      ...
      append(boost::lexical_cast<string>(number_of_childs)).
      append("; ++i) {\n").
...
template <class TValue>
string DecisionVertex<TValue>::getDefaultAlias() const {
