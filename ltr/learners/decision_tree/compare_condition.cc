...
string CompareCondition::getDefaultAlias() const {
  return "Decision Tree";
}

double CompareCondition::value(const ltr::Object &object) const {
  INFO("Finding value of object.");
  if (weak_condition_ == NULL) {
    throw std::logic_error("no weak condition");
  }

  double weak_value = weak_condition_->value(object);
  switch (compare_type_) {
    case EQUAL:
      return utility::DoubleEqual(weak_value, compare_number_);
    case GREATER:
      return utility::DoubleMore(weak_value, compare_number_);
    case LESS:
      return utility::DoubleLess(weak_value, compare_number_);
...
CompareCondition::Ptr CompareConditionPtr(
  Condition::Ptr weak_condition, CompareType type, double number) {
    return CompareCondition::Ptr(
      new CompareCondition(weak_condition, type, number));
}
};
};
