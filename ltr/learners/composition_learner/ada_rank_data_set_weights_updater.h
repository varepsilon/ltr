...
void AdaRankDataSetWeightsUpdater<TElement>::updateWeights(
  ...
  vector<double> weighted_measure_exps(data->size());
  double sum_of_weighted_exps = 0.0;
  ...
  for (size_t element_index = 0; element_index < data->size(); ++element_index) {
    ...
    weighted_measure_exps[element_index] = measure_value_exp * data->getWeight(element_index);
    sum_of_weighted_exps += weighted_measure_exps[element_index];
  }

  for (size_t element_index = 0; element_index < data->size(); ++element_index) {
    data->setWeight(element_index, weighted_measure_exps[element_index] / sum_of_weighted_exps);
  }
}
};
};
