...
TEST(SumSquaresFunctionTest, SumSquaresFunctionValueTest) {
  SumSquaresFunction::Ptr sum_squares_function(new SumSquaresFunction(4));
  ...
  SumSquaresFunction::Ptr sum_squares_function1(new SumSquaresFunction(2));
...
TEST(SumSquaresFunctionTest, SumSquaresExceptionTest) {
  EXPECT_ANY_THROW(SumSquaresFunction::Ptr sum_squares_function(
                       new SumSquaresFunction(0)));
  EXPECT_NO_THROW(SumSquaresFunction::Ptr sum_squares_function(
                      new SumSquaresFunction(2)));
}

TEST(EasomFunctionTest, EasomFunctionValueTest) {
  EasomFunction::Ptr easom_function(new EasomFunction);
  Point point(2);
  point << PI, PI;
...
TEST(RosenbrockFunctionTest, RosenbrockFunctionValueTest) {
  RosenbrockFunction::Ptr rosenbrock_function(new RosenbrockFunction(4));
  Point point(4);
  point.setZero();
...
TEST(RosenbrockFunctionTest, RosenbrockFunctionHessianTest) {
  RosenbrockFunction::Ptr rosenbrock_function(new RosenbrockFunction(2));
  Point point(2);
  Matrix hessian(2, 2);
  Matrix true_hessian(2, 2);
  ...
  RosenbrockFunction::Ptr rosenbrock_function1(new RosenbrockFunction(3));
...
TEST(GriewankFunctionTest, GriewankFunctionValueTest) {
  GriewankFunction::Ptr griewank_function(new GriewankFunction(5));
  Point point(5);
  point.setZero();

  EXPECT_DOUBLE_EQ(0.0, griewank_function->computeValue(point));

  GriewankFunction::Ptr griewank_function1(new GriewankFunction(2));
  Point point1(2);
...
TEST(GriewankFunctionTest, GriewankFunctionHessianTest) {
  GriewankFunction::Ptr griewank_function(new GriewankFunction(2));
  Point point(2);
  Matrix true_hessian(2, 2);
  Matrix hessian(2, 2);
...
TEST(TridFunctionTest, TridFunctionValueTest) {
  TridFunction::Ptr trid_function(new TridFunction(4));
  Point point(4);
...
TEST(TridFunctionTest, TridFunctionGradientTest) {
  ...
  TridFunction::Ptr trid_function1(new TridFunction(4));
  Point point1(4);
  Vector gradient1(4);
...
TEST(TridFunctionTest, TridFunctionExceptionTest) {
  EXPECT_ANY_THROW(TridFunction::Ptr trid_function(new TridFunction(1)));
  EXPECT_NO_THROW(TridFunction::Ptr trid_function(new TridFunction(2)));
}

TEST(RastriginFunctionTest, RastriginFunctionValueTest) {
  RastriginFunction::Ptr rastrigin_function(new RastriginFunction(10));
  Point point(10);
  point.setZero();

  EXPECT_DOUBLE_EQ(0.0, rastrigin_function->computeValue(point));

  RastriginFunction::Ptr rastrigin_function1(new RastriginFunction(2));
  Point point1(2);
  point1 << 1.0 / 4.0, 1.0 / 4.0;

  double value = 1.0 / 8.0 + 20.0 - 20.0 * cos(PI / 2.0);

  EXPECT_DOUBLE_EQ(value, rastrigin_function1->computeValue(point1));
}

TEST(RastriginFunctionTest, RastriginFunctionGradientTest) {
  RastriginFunction::Ptr rastrigin_function(new RastriginFunction(10));
  Point point(10);
  Vector true_gradient(10);
  Vector gradient(10);

  point.setZero();
  true_gradient.setZero();
  rastrigin_function->computeGradient(point, &gradient);
  EXPECT_TRUE(gradient.isApprox(true_gradient, 0.01));
}

TEST(RastriginFunctionTest, RastriginFunctionHessianTest) {
  RastriginFunction::Ptr rastrigin_function(new RastriginFunction(2));
  Point point(2);
  Matrix true_hessian(2, 2);
  Matrix hessian(2, 2);
  ...
  point.setZero();
  true_hessian.setZero();
  true_hessian(0, 0) = 2.0 + 40.0 * pow(PI, 2);
  true_hessian(1, 1) = true_hessian(0, 0);

  rastrigin_function->computeHessian(point, &hessian);
